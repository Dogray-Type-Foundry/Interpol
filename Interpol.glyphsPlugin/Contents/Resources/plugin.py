...
DEBUG_INTERPOLATE = False  # Set to True to enable debug logging for special layers
...
# =============================================================================
# Curve Statistics Helper - StatisticsPen-based curve analysis for dekinking
# =============================================================================


class CurveStatisticsHelper:
    """
    Helper class that uses fontTools StatisticsPen to analyze curve segments
    and find optimal dekinking solutions that preserve curve shape.

    The StatisticsPen computes statistical properties of a curve:
    - area: Signed area enclosed (indicates curve "weight" and direction)
    - meanX, meanY: Centroid of the curve
    - stddevX, stddevY: Spread/variance in each direction
    - correlation: How X and Y coordinates relate
    - slant: Overall slant angle

    These statistics capture curve "behavior" more comprehensively than
    point sampling at fixed t values.
    """

    # Weights for each statistic when computing curve similarity
    # These are tuned to prioritize what matters most for visual similarity
    STAT_WEIGHTS = {
        "area": 1.0,  # Overall shape/size
        "meanX": 0.8,  # Position - less important if we're adjusting position
        "meanY": 0.8,  # Position - less important if we're adjusting position
        "stddevX": 1.5,  # Spread - very important for curve shape
        "stddevY": 1.5,  # Spread - very important for curve shape
        "correlation": 1.2,  # Shape characteristic
        "slant": 0.6,  # Less weight - can vary between masters naturally
    }

    @staticmethod
    def create_segment_pen(p0, p1, p2, p3):
        """
        Create a RecordingPen that draws a single cubic bezier segment.
        The segment is closed to ensure StatisticsPen computes proper area.

        Args:
            p0, p1, p2, p3: Control points as (x, y) tuples

        Returns:
            RecordingPen with the segment recorded
        """
        pen = RecordingPen()
        pen.moveTo(p0)
    ...
    @staticmethod
    def create_line_segment_pen(p0, p1, reference_point=None):
        """
        Create a RecordingPen that draws a line segment as a triangle.
        Lines have no area, so we create a small triangle to get meaningful stats.

        Args:
            p0: Start point (x, y)
            p1: End point (x, y)
            reference_point: Optional third point to form triangle. If None,
                           creates a point perpendicular to the line.
        ...
        if reference_point is None:
            ...
            if length > 0:
                # Perpendicular vector, scaled to ~5% of line length
                offset = max(1.0, length * 0.05)
                reference_point = (
                    (p0[0] + p1[0]) / 2 - dy / length * offset,
                    (p0[1] + p1[1]) / 2 + dx / length * offset,
                )
            else:
                reference_point = (p0[0] + 1, p0[1] + 1)

        pen.lineTo(reference_point)
        pen.closePath()
        return pen

    @staticmethod
    def get_segment_stats(pen):
        """
        Compute statistics for a segment using StatisticsPen.

        Args:
            pen: RecordingPen with segment recorded

        Returns:
            Dict of statistics, or None if computation fails
        """
        try:
            stats_pen = StatisticsPen(glyphset=None)
            pen.replay(stats_pen)

            return {
                "area": stats_pen.area,
                "meanX": stats_pen.meanX,
                "meanY": stats_pen.meanY,
                "stddevX": stats_pen.stddevX,
                "stddevY": stats_pen.stddevY,
                "correlation": (
                    stats_pen.correlation if hasattr(stats_pen, "correlation") else 0
                ),
                "slant": stats_pen.slant if hasattr(stats_pen, "slant") else 0,
            }
        except Exception:
            return None

    @staticmethod
    def compute_similarity(stats1, stats2, position_sensitive=True):
        """
        Compute similarity between two sets of curve statistics.

        Args:
            stats1: First statistics dict
            stats2: Second statistics dict
            position_sensitive: If False, ignore meanX/meanY differences
        ...
        for key, weight in CurveStatisticsHelper.STAT_WEIGHTS.items():
            if key not in stats1 or key not in stats2:
                continue

            # Skip position stats if not position sensitive
            if not position_sensitive and key in ("meanX", "meanY"):
                continue
            ...
            # Compute similarity based on the statistic type
            if key == "area":
                # Area: compare absolute values with sign consideration
                abs_max = max(abs(v1), abs(v2), 1.0)
                # Sign mismatch is a big problem
                if (v1 > 0) != (v2 > 0) and abs(v1) > 1 and abs(v2) > 1:
                    similarity = 0.0
                else:
                    similarity = 1.0 - min(1.0, abs(v1 - v2) / abs_max)
            elif key in ("stddevX", "stddevY"):
                # Standard deviation: relative comparison
                max_val = max(abs(v1), abs(v2), 1.0)
                similarity = 1.0 - min(1.0, abs(v1 - v2) / max_val)
            elif key in ("meanX", "meanY"):
                # Position: allow some variance based on absolute values
                range_val = max(abs(v1), abs(v2), 100.0)
                similarity = 1.0 - min(1.0, abs(v1 - v2) / range_val)
            elif key == "correlation":
                # Correlation: direct difference (both in -1 to 1 range)
                similarity = 1.0 - min(1.0, abs(v1 - v2) / 2.0)
            elif key == "slant":
                # Slant: angular difference
                angle_diff = abs(v1 - v2)
                similarity = 1.0 - min(1.0, angle_diff / 45.0)  # 45° = 0 similarity
            else:
                continue
        ...
        return total_weighted_similarity / total_weight

    @staticmethod
    def get_curve_segment_from_node(node, path, direction):
        """
        Extract a curve segment adjacent to a node.

        Args:
            node: The smooth on-curve node
            path: The GSPath containing the node
            direction: "prev" (curve coming in) or "next" (curve going out)

        Returns:
            Dict with segment info, or None if not a curve segment:
            {
                'p0', 'p1', 'p2', 'p3': Control points as (x, y) tuples
                'is_curve': True if cubic bezier, False if line
                'handle_node': The handle connected to our node
                'opp_handle_node': The opposite handle
                'opp_oncurve_node': The opposite on-curve node
            }
        """
        ...
        if direction == "prev":
            # Curve: prev_oncurve -> opp_handle -> our_handle -> node
            our_handle_idx = (node_idx - 1) % num_nodes
            our_handle = nodes[our_handle_idx]

            # Check if our handle is off-curve (indicates a curve segment)
            if our_handle.type != GSOFFCURVE:
                # Line segment
                p0 = (our_handle.position.x, our_handle.position.y)
                p1 = (node.position.x, node.position.y)
                return {
                    "p0": p0,
                    "p1": p1,
                    "p2": None,
                    ...
                    "is_curve": False,
                    "handle_node": None,
                    "opp_handle_node": None,
                    "opp_oncurve_node": our_handle,
                }

            # Find opposite handle (should be just before our handle)
            opp_handle_idx = (our_handle_idx - 1) % num_nodes
            opp_handle = nodes[opp_handle_idx]

            if opp_handle.type != GSOFFCURVE:
                # This is a half-curve (only one off-curve handle)
                p0 = (opp_handle.position.x, opp_handle.position.y)
                p1 = p0  # No off-curve handle on this side
                ...
                return {
                    ...
                    "p3": p3,
                    "is_curve": True,
                    "handle_node": our_handle,
                    "opp_handle_node": None,
                    "opp_oncurve_node": opp_handle,
                }

            # Find prev on-curve
            prev_oncurve_idx = (opp_handle_idx - 1) % num_nodes
            while (
                nodes[prev_oncurve_idx].type == GSOFFCURVE
                and prev_oncurve_idx != node_idx
            ):
            ...
            return {
                ...
                "is_curve": True,
                "handle_node": our_handle,
                "opp_handle_node": opp_handle,
                "opp_oncurve_node": prev_oncurve,
            }

        else:  # direction == "next"
            # Curve: node -> our_handle -> opp_handle -> next_oncurve
            our_handle_idx = (node_idx + 1) % num_nodes
            our_handle = nodes[our_handle_idx]
            ...
            if opp_handle.type != GSOFFCURVE:
                ...
                p1 = (our_handle.position.x, our_handle.position.y)
                p2 = (
                    opp_handle.position.x,
                    opp_handle.position.y,
                )  # Actually the on-curve
                p3 = p2
                return {
                    "p0": p0,
                    "p1": p1,
            ...
            # Find next on-curve
            next_oncurve_idx = (opp_handle_idx + 1) % num_nodes
            while (
                nodes[next_oncurve_idx].type == GSOFFCURVE
                and next_oncurve_idx != node_idx
            ):
    ...
    @staticmethod
    def compute_original_segment_stats(node, path):
        """
        Compute statistics for the original curve segments adjacent to a node.
        ...
        Returns:
            Dict with 'prev' and 'next' segment stats, or None for each if not a curve
        """
        ...
        for direction in ["prev", "next"]:
            ...
            if seg_info["is_curve"]:
                pen = CurveStatisticsHelper.create_segment_pen(
                    seg_info["p0"], seg_info["p1"], seg_info["p2"], seg_info["p3"]
                )
            else:
                pen = CurveStatisticsHelper.create_line_segment_pen(
                    seg_info["p0"], seg_info["p1"]
                )
    ...
    @staticmethod
    def compute_modified_segment_stats(
        node, path, new_node_pos, new_prev_pos=None, new_next_pos=None
    ):
        """
        Compute statistics for curve segments after modifying positions.

        Args:
            node: The smooth on-curve node (for structure reference)
            path: The GSPath containing the node
            new_node_pos: New position for the on-curve node (x, y)
            new_prev_pos: New position for prev handle, or None to keep original
            new_next_pos: New position for next handle, or None to keep original

        Returns:
            Dict with 'prev' and 'next' segment stats
        """
        result = {"prev": None, "next": None}
        ...
        for direction in ["prev", "next"]:
            ...
            if seg_info["is_curve"]:
                # Build modified control points
                if direction == "prev":
                    # Curve: p0 (fixed) -> p1 (opp_handle) -> p2 (our handle) -> p3 (our node)
                    p0 = seg_info["p0"]
                    p1 = seg_info["p1"]  # Opposite handle - could compensate
                    p2 = new_prev_pos if new_prev_pos else seg_info["p2"]
                    p3 = new_node_pos
                else:
                    # Curve: p0 (our node) -> p1 (our handle) -> p2 (opp_handle) -> p3 (fixed)
                    p0 = new_node_pos
                    p1 = new_next_pos if new_next_pos else seg_info["p1"]
                    p2 = seg_info["p2"]  # Opposite handle - could compensate
                    p3 = seg_info["p3"]

                pen = CurveStatisticsHelper.create_segment_pen(p0, p1, p2, p3)
            else:
                # Line segment
                if direction == "prev":
                    p0 = seg_info["p0"]
                    ...
                    p0 = new_node_pos
                    p1 = seg_info["p1"]
                pen = CurveStatisticsHelper.create_line_segment_pen(p0, p1)
    ...
    def evaluate_dekink_quality(original_stats, modified_stats):
        """
        Evaluate how much curve quality is preserved after dekinking.

        Args:
            original_stats: Dict with 'prev' and 'next' original segment stats
            modified_stats: Dict with 'prev' and 'next' modified segment stats

        Returns:
            Quality score from 0.0 (completely changed) to 1.0 (identical)
        """
        total_similarity = 0.0
        count = 0

        for direction in ["prev", "next"]:
            orig = original_stats.get(direction)
            mod = modified_stats.get(direction)

            if orig and mod and orig.get("stats") and mod.get("stats"):
                sim = CurveStatisticsHelper.compute_similarity(
                    orig["stats"], mod["stats"], position_sensitive=False
                )
    ...
    def compute_cross_master_similarity(node_info_list):
        """
        Compute how similar the curves are across all masters.

        Used to determine if aggressive dekinking is safe (similar curves)
        or if we need to be cautious (different curves).

        Args:
            node_info_list: List of dicts, each containing:
                - 'node': The node
                - 'path': The path
        ...
        # Collect stats for each master
        all_stats = []
        for info in node_info_list:
            stats = CurveStatisticsHelper.compute_original_segment_stats(
                info["node"], info["path"]
            )
            all_stats.append(stats)

        # Compare each master to the first master
        ref_stats = all_stats[0]
        total_similarity = 0.0
        comparisons = 0

        for other_stats in all_stats[1:]:
            for direction in ["prev", "next"]:
                ref = ref_stats.get(direction)
                other = other_stats.get(direction)

                if ref and other and ref.get("stats") and other.get("stats"):
                    sim = CurveStatisticsHelper.compute_similarity(
                        ref["stats"], other["stats"], position_sensitive=False
                    )
                    total_similarity += sim
                    comparisons += 1
...
class SynchronizationHelper:
    ...
    @staticmethod
    def find_handle_scale_for_midpoint(
        p0,
        p1_dir,
        p2_fixed,
        p3,
    ...
    @staticmethod
    def find_handle_scale_for_midpoint_p2(
        p0,
        p1_fixed,
        p2_dir,
        p3,
    ...
    @staticmethod
    def calculate_positions_for_blend(
        node, path, target_ratio, blend_factor, off_curve_change=0.5
    ):
        """
        ...
        off_curve_count = sum(
            1 for pt in [prev_node, next_node] if pt.type == GSOFFCURVE
        )
    ...
    @staticmethod
    def find_optimal_blend_for_node(
        node, path, target_ratio, max_deviation=25.0, steps=21
    ):
        """
        ...
        initial_kink = SynchronizationHelper.calculate_kink_severity(
    ...
    @staticmethod
    def find_optimal_ratio_across_masters(
        node, path, path_idx, node_idx, glyph, font, max_deviation=25.0
    ):
        """
        ...
        for master in font.masters:
            ...
            try:
                ...
                current_ratio = SynchronizationHelper.calc_ratio(
        ...
        curve_similarity = (
            SynchronizationHelper.calculate_cross_master_curve_similarity(master_data)
        )
    ...
    @staticmethod
    def calculate_deviation_for_ratio(
        node, path, target_ratio, prev_info, next_info, prev_samples, next_samples
    ):
        """
    ...
    def sync_ratios_auto(selected_nodes_info, font, max_deviation=40.0):
        ...
        for node, path, path_idx, node_idx in selected_nodes_info:
            ...
            optimal_ratio, adjusted_max_dev = (
                SynchronizationHelper.find_optimal_ratio_across_masters(
                ...
                try:
                    ...
                    if prev_info and prev_node.type == GSOFFCURVE:
                        ...
                        if abs(p1_dir[0]) > 0.1 or abs(p1_dir[1]) > 0.1:
                            best_scale = (
                                SynchronizationHelper.find_handle_scale_for_midpoint(
                    ...
                    if next_info and next_node.type == GSOFFCURVE:
                        ...
                        if abs(p2_dir[0]) > 0.1 or abs(p2_dir[1]) > 0.1:
                            best_scale = (
                                SynchronizationHelper.find_handle_scale_for_midpoint_p2(
    ...
    @staticmethod
    def sync_ratios_auto_statistics(selected_nodes_info, font, quality_threshold=0.85):
        """
        ...
        for node, path, path_idx, node_idx in selected_nodes_info:
            ...
            for master in font.masters:
                ...
                try:
                    ...
                    # Get curve statistics for this master
                    original_stats = (
                        CurveStatisticsHelper.compute_original_segment_stats(
                            corresponding, master_path
                        )
                    )
            ...
            # Check ratio spread - if already close, skip
            ratios = [d["ratio"] for d in master_data]
            ...
            for data in master_data:
                ...
                for iteration in range(15):  # Binary search iterations
                    ...
                    modified_stats = (
                        CurveStatisticsHelper.compute_modified_segment_stats(
                            corresponding,
                            master_path,
                            positions["node_pos"],
                            (
                    ...
                    # Evaluate quality
                    quality = CurveStatisticsHelper.evaluate_dekink_quality(
    ...
    @staticmethod
    def calculate_preview_points_auto_statistics(
        selected_nodes_info, font, quality_threshold=0.85
    ):
        """
        ...
        for ix, master in enumerate(font.masters):
            ...
            preview_points[ix] = list(
                GlyphInterpolator.extract_points_from_layer(decomposed)
            )
        ...
        for node, path, path_idx, node_idx in selected_nodes_info:
            ...
            for data in master_data:
                ...
                prev_idx_in_path = (corr_idx - 1) % num_nodes
                next_idx_in_path = (corr_idx + 1) % num_nodes
                prev_flat_idx = node_to_point.get((path_idx, prev_idx_in_path))
                next_flat_idx = node_to_point.get((path_idx, next_idx_in_path))

                if node_flat_idx is not None and node_flat_idx < len(
                ...
                for direction in ["prev", "next"]:
                    ...
                    if abs(orig_handle_dir[0]) > 0.1 or abs(orig_handle_dir[1]) > 0.1:
                        ...
                        if opp_flat_idx is not None and opp_flat_idx < len(
                            ...
                            preview_points[master_idx][opp_flat_idx] = (
                                round(new_opp_pos[0]),
                                round(new_opp_pos[1]),
                            )
    ...
    def sync_ratios(
        ...
        font,
        mode="follow",
        off_curve_change=0.15,
        compensate_curves=False,
    ):
        """
            ...
            if mode == "follow":
                ...
                for master in font.masters:
                    ...
                    if corresponding:
                        try:
                            master_path = master_layer.paths[path_idx]
                            ratio = SynchronizationHelper.calc_ratio(
                                corresponding, master_path
                            )
                            ratios.append(ratio)
                        except Exception:
            ...
            for master in font.masters:
                ...
                try:
                    ...
                    if compensate_curves:
                        ...
                        if (
                            prev_info
                            and prev_info["opp_oncurve_idx"] not in selected_in_path
                        ):
                        ...
                        if (
                            next_info
                            and next_info["opp_oncurve_idx"] not in selected_in_path
                        ):
        ...
        for node, path, path_idx, node_idx in selected_nodes_info:
            ...
            for master_idx, master in enumerate(font.masters):
                ...
                try:
                    ...
                    new_prev_x = SynchronizationHelper.lerp(in_factor, p2[0], p3[0])
                    new_prev_y = SynchronizationHelper.lerp(in_factor, p2[1], p3[1])
                    new_prev_x = SynchronizationHelper.lerp(
                        off_soften, p1[0], new_prev_x
                    )
                    new_prev_y = SynchronizationHelper.lerp(
                        off_soften, p1[1], new_prev_y
                    )

                    new_next_x = SynchronizationHelper.lerp(out_factor, p1[0], p2[0])
                    new_next_y = SynchronizationHelper.lerp(out_factor, p1[1], p2[1])
                    new_next_x = SynchronizationHelper.lerp(
                        off_soften, p3[0], new_next_x
                    )
                    new_next_y = SynchronizationHelper.lerp(
                        off_soften, p3[1], new_next_y
                    )
                    ...
                    if (
                        prev_node.type == GSOFFCURVE
                        and prev_flat_idx is not None
                        and prev_flat_idx < len(preview_points[master_idx])
                    ):
                        preview_points[master_idx][prev_flat_idx] = (
                            round(new_prev_x),
                            round(new_prev_y),
                    ...
                    if (
                        next_node.type == GSOFFCURVE
                        and next_flat_idx is not None
                        and next_flat_idx < len(preview_points[master_idx])
                    ):
                        preview_points[master_idx][next_flat_idx] = (
                            round(new_next_x),
                            round(new_next_y),
...
class SyncRatiosPanel:
    ...
    def _build_ui(self):
        ...
        y -= 25
        mainModeHeight = 38
        mainModeContainer = NSView.alloc().initWithFrame_(
            NSMakeRect(15, y - mainModeHeight + 18, width - 30, mainModeHeight)
        )
        ...
        self._controls["autoRadio"] = self._create_radio_button(
            mainModeContainer, "Auto (Statistics-based)", (0, 0, width - 30, 18), False
        )
        y -= mainModeHeight
        ...
        y -= 25
        targetModeHeight = 38
        targetModeContainer = NSView.alloc().initWithFrame_(
            NSMakeRect(15, y - targetModeHeight + 18, width - 30, targetModeHeight)
        )
    ...
    def _update_manual_controls_visibility(self):
        ...
        is_manual = (
            self._controls.get("manualRadio")
            and self._controls["manualRadio"].state() == 1
        )
    ...
    def _on_sync_clicked(self, sender):
        ...
        try:
            ...
            selected_nodes = SynchronizationHelper.get_selected_smooth_oncurve_nodes(
                layer
            )
            ...
            filtered_nodes = SynchronizationHelper.filter_nodes_with_kinks(
                selected_nodes, layer, font
            )
            ...
            try:
                if is_auto_mode:
                    # Use the improved statistics-based auto dekinking
                    synced_count = SynchronizationHelper.sync_ratios_auto_statistics(
                        filtered_nodes, font, quality_threshold=0.85
                    )
                else:
                    mode = (
                        "follow"
                        if self._controls["matchRadio"].state() == 1
                        else "average"
                    )
    ...
    def _update_selection_count(self):
        ...
        try:
            ...
            if total_count == 0:
                self._controls["countLabel"].setStringValue_(
                    "No smooth points selected"
                )
                return
            ...
            if kinked_count == 0:
                self._controls["countLabel"].setStringValue_(
                    f"{total_count} selected, 0 with kinks"
                )
                ...
                if kinked_count == 1:
                    ...
                    self._controls["countLabel"].setStringValue_(
                        f"{kinked_count} points with kinks"
                    )
            else:
                self._controls["countLabel"].setStringValue_(
                    f"{kinked_count} of {total_count} with kinks"
                )
        except Exception:
            try:
    ...
    def _update_preview(self):
        ...
        try:
            ...
            if is_auto_mode:
                # Use the improved statistics-based auto preview
                preview_points = (
                    SynchronizationHelper.calculate_preview_points_auto_statistics(
                        filtered_nodes, font, quality_threshold=0.85
                    )
                )
            else:
                mode = (
                    "follow" if self._controls["matchRadio"].state() == 1 else "average"
                )
                off_curve_change = self._controls["balanceSlider"].doubleValue()
                compensate_curves = self._controls["compensateCheck"].state() == 1
            ...
            if preview_points:
                ...
                try:
                    cache = InterpolationCache()
                    SyncRatiosPanel._preview_content_hash = (
                        cache.get_glyph_content_hash(glyph, font)
                    )
                except Exception:
...
DEFAULT_DEKINK_PREVIEW_COLOR = (
    0.0,
    0.7,
    0.2,
    1.0,
)  # Used for dekink window preview (green)
DEFAULT_TOOL_SHORTCUT = "a"
DEFAULT_USE_DOTTED_LINES = True  # True = dotted, False = solid


def _load_color_from_defaults(
    key: str, default: Tuple[float, ...]
) -> Tuple[float, ...]:
...
class InterpolateConfig:
    ...
    def get_kink_indicator_color() -> Tuple[float, ...]:
        ...
        return _load_color_from_defaults(
            KEY + ".color.kink_indicator", DEFAULT_KINK_INDICATOR_COLOR
        )
    ...
    def get_dekink_preview_color() -> Tuple[float, ...]:
        ...
        return _load_color_from_defaults(
            KEY + ".color.dekink_preview", DEFAULT_DEKINK_PREVIEW_COLOR
        )
...
class InterpolationCache:
    ...
    def get_scalars_hash(
        self, master_scalars: List[float]
    ...
    def get_cached_path(
        self,
        glyph_name: str,
        scalars_hash: Optional[Tuple],
        content_hash: Optional[Tuple],
    ) -> Optional[Tuple]:
    ...
    def set_path(
        ...
        keys_to_remove = [
            k for k in self._paths if k[0] == glyph_name and k != cache_key
        ]
    ...
    def set_font_metrics(
        self, font_signature: str, metrics: Tuple[float, float, float]
    ) -> None:
...
class GlyphInterpolator:
    ...
    def interpolate_points(
        glyph_points: Dict[int, Any], model: Any, master_scalars: List[float]
    ) -> Optional[Any]:
        """
    ...
    def build_bezier_path(
        reference_layer: Any, interpolated_points: Any
    ...
    def interpolate_anchors(
        glyph: Any, font: Any, master_scalars: List[float]
        ...
        try:
            ...
            anchor_names_sets = [
                set(a.name for a in layer.anchors) for layer in master_layers
            ]
    ...
    def get_interpolated_kerning(
        left_glyph: Any, right_glyph: Any, font: Any, master_scalars: List[float]
    ) -> float:
        """
...
# =============================================================================
# Special Layer Helpers (Intermediate & Alternate Layers)
# =============================================================================
...
class KinkDetector:
    ...
    @staticmethod
    def check_compatible_angles(
        glyph, layer_ids: List[str], shape_index: int, node_index: int
    ) -> Tuple[bool, float]:
        """
        ...
        for layer_id in layer_ids:
            ...
            if current_path:
                try:
                    ...
                    continue
                if current_node:
                    prev_node, next_node = KinkDetector.get_prev_next_nodes(
                        current_path, node_index
                    )
    ...
    @staticmethod
    def check_compatible_proportions(
        glyph,
        layer_ids: List[str],
        shape_index: int,
        node_index: int,
        original_hypot: Tuple[float, float],
    ) -> bool:
        ...
        for layer_id in layer_ids:
            ...
            if current_path:
                ...
                if current_node:
                    ...
                    round_error = 0.5
                    if not (
                        proportion1 >= proportion2 - round_error
                        and proportion1 <= proportion2 + round_error
                    ):
                        return False
    ...
    def find_potential_kinks(
        ...
        for path_index, path in enumerate(layer.shapes):
            ...
            for node_index, node in enumerate(nodes):
                ...
                compatible_angles, max_angle_diff = (
                    KinkDetector.check_compatible_angles(
    ...
    def precompute_kinks(
        ...
        try:
            ...
            for sample_location in sample_locations:
                ...
                interpolated_points = model.interpolateFromValuesAndScalars(
                    all_points, scalars
                )
                ...
                for node_id, path_idx, node_idx in kink_nodes:
                    ...
                    if (
                        prev_flat_idx is None
                        or next_flat_idx is None
                        or prev_flat_idx >= len(interp_list)
                        or next_flat_idx >= len(interp_list)
                    ):
                        continue
...
class DrawingHelpers:
    ...
    def draw_kink_indicator(
        ...
        kink_color = (
            kink_indicator_color[0],
            kink_indicator_color[1],
            kink_indicator_color[2],
        )
    ...
    def draw_tooltip(
        ...
        x, y = position
        bgRect = NSMakeRect(
            x - padding, y - padding, textWidth + 2 * padding, textHeight + 2 * padding
        )
        ...
        try:
            ...
            if Glyphs.font:
                ...
                if Glyphs.font.masters:
                    ...
                    descender = (
                        master.descender if hasattr(master, "descender") else -250
                    )
...
class InterpolateState:
    ...
    def __init__(self):
        ...
        self.potential_kinks = (
            []
        )  # List of (path_idx, node_idx, node_id, max_angle_diff) from detection
        self.interpolated_kinks = (
            []
        )  # List of (x, y, angle_diff, max_angle_diff, angle_deg) for display
        ...
        defaults = NSUserDefaults.standardUserDefaults()
        self.show_preview = (
            defaults.boolForKey_(KEY + ".show_preview")
            if defaults.objectForKey_(KEY + ".show_preview") is not None
            else True
        )
        self.show_nodes = (
            defaults.boolForKey_(KEY + ".show_nodes")
            if defaults.objectForKey_(KEY + ".show_nodes") is not None
            else False
        )
        self.show_fill = (
            defaults.boolForKey_(KEY + ".show_fill")
            if defaults.objectForKey_(KEY + ".show_fill") is not None
            else True
        )
        self.show_anchors = (
            defaults.boolForKey_(KEY + ".show_anchors")
            if defaults.objectForKey_(KEY + ".show_anchors") is not None
            else False
        )
        ...
        self.show_compare_line = (
            defaults.boolForKey_(KEY + ".show_compare_line")
            if defaults.objectForKey_(KEY + ".show_compare_line") is not None
            else False
        )

        # Follow selected master position once on master switch
        self.follow_masters = (
            defaults.boolForKey_(KEY + ".follow_masters")
            if defaults.objectForKey_(KEY + ".follow_masters") is not None
            else False
        )

        # Bubble scale (1.0 = same size as edit view, 0.5 = 50%, 2.0 = 200%)
        self.bubble_scale = (
            defaults.floatForKey_(KEY + ".bubble_scale")
            if defaults.objectForKey_(KEY + ".bubble_scale") is not None
            else 1.0
        )
        ...
        # Show kinks toggle
        self.show_kinks = (
            defaults.boolForKey_(KEY + ".show_kinks")
            if defaults.objectForKey_(KEY + ".show_kinks") is not None
            else False
        )

        # Dim the main layer to make interpolation preview more visible
        self.dim_layer = (
            defaults.boolForKey_(KEY + ".dim_layer")
            if defaults.objectForKey_(KEY + ".dim_layer") is not None
            else False
        )

        # Spacebar preview shows interpolated glyphs instead of current master
        self.space_preview_interpolated = (
            defaults.boolForKey_(KEY + ".space_preview_interpolated")
            if defaults.objectForKey_(KEY + ".space_preview_interpolated") is not None
            else False
        )

        # Use intermediate (brace) layers in interpolation
        self.use_intermediate_layers = (
            defaults.boolForKey_(KEY + ".use_intermediate_layers")
            if defaults.objectForKey_(KEY + ".use_intermediate_layers") is not None
            else True  # Enabled by default
        )

        # Use alternate (bracket) layers in interpolation
        self.use_alternate_layers = (
            defaults.boolForKey_(KEY + ".use_alternate_layers")
            if defaults.objectForKey_(KEY + ".use_alternate_layers") is not None
            else True  # Enabled by default
        )
    ...
    def save_defaults(self):
        ...
        defaults.setBool_forKey_(
            self.space_preview_interpolated, KEY + ".space_preview_interpolated"
        )
        defaults.setBool_forKey_(
            self.use_intermediate_layers, KEY + ".use_intermediate_layers"
        )
        defaults.setBool_forKey_(
            self.use_alternate_layers, KEY + ".use_alternate_layers"
        )
        try:
            defaults.synchronize()
        except Exception:
    ...
    def build_model(self):
        ...
        if self.model and self._model_master_ids == current_master_ids:
            debug_log(
                "build_model(): Already have model with matching masters, skipping"
            )
            return
        ...
        debug_log(
            f"build_model(): Building model for font with axes {list(self.axis_min_max.keys())}"
        )
        ...
        if self.current_location:
            self.master_scalars = self.model.getMasterScalars(self.current_location)
            debug_log(
                f"build_model(): Initialized master_scalars with {len(self.master_scalars)} values"
            )
        else:
            ...
            if locations:
                ...
                self.master_scalars = self.model.getMasterScalars(self.current_location)
                debug_log(
                    f"build_model(): No current_location, using first master location. master_scalars has {len(self.master_scalars)} values"
                )

    def get_glyph_model(self, glyph, font):
        """
        Get the appropriate interpolation model and data for a glyph.

        If the glyph has intermediate or active alternate layers, builds a glyph-specific
        model that includes those sources. Otherwise, returns the master-only model.

        Respects both use_intermediate_layers and use_alternate_layers settings.

        Args:
            glyph: The GSGlyph to get model data for
            font: The GSFont containing the glyph

        Returns:
            tuple: (model, glyph_points_dict, widths_list, scalars, reference_decomposed)
                   or (None, None, None, None, None) on failure
        """
        if not glyph or not font or not self.axis_min_max or not self.current_location:
            if DEBUG_INTERPOLATE:
                print(
                    f"[Interpol] get_glyph_model: early return - glyph={glyph is not None}, font={font is not None}, "
                    f"axis_min_max={self.axis_min_max is not None}, current_location={self.current_location is not None}"
                )
            return None, None, None, None, None

        # If both special layer types are disabled, skip special layer collection entirely
        if not self.use_intermediate_layers and not self.use_alternate_layers:
            if self.model is None:
                return None, None, None, None, None

            # Extract points from masters only
            glyph_points = {}
            widths = []
            decomposed_layers = []
            ...
            for ix, master in enumerate(font.masters):
                ...
                glyph_points[ix] = GlyphInterpolator.extract_points_from_layer(
                    decomposed
                )
            ...
            scalars = self.model.getMasterScalars(self.current_location)
            return self.model, glyph_points, widths, scalars, decomposed_layers[0]

        if DEBUG_INTERPOLATE:
            print(f"[Interpol] get_glyph_model: Processing glyph '{glyph.name}'")
            print(f"[Interpol]   current_location: {self.current_location}")
            print(f"[Interpol]   axis_min_max: {self.axis_min_max}")
            print(
                f"[Interpol]   use_intermediate_layers: {self.use_intermediate_layers}, use_alternate_layers: {self.use_alternate_layers}"
            )

        # Try to get glyph-specific sources (masters + intermediate + active alternates)
        source_layers, source_locations, uses_special_layers = (
            SpecialLayerHelper.collect_interpolation_sources(
                glyph,
                font,
                self.current_location,
                self.axis_min_max,
                include_intermediates=self.use_intermediate_layers,
                include_alternates=self.use_alternate_layers,
            )
        )

        if DEBUG_INTERPOLATE:
            print(
                f"[Interpol]   collected {len(source_layers)} sources, {len(source_locations)} locations, uses_special={uses_special_layers}"
            )
            for i, (lyr, loc) in enumerate(zip(source_layers, source_locations)):
                print(f"[Interpol]     source[{i}]: layer='{lyr.name}', location={loc}")

        # If we only have master layers (no special layers), use the existing master model
        num_masters = len(font.masters)
        if not uses_special_layers and len(source_layers) == num_masters:
            # No special layers found - use the standard master model
            # Note: We still need to collect the data even for master-only case
            if self.model is None:
                return None, None, None, None, None

            # Extract points from masters
            glyph_points = {}
            widths = []
            decomposed_layers = []
        ...
        # We have special layers - build a glyph-specific model (or use source layers directly)
        if DEBUG_INTERPOLATE:
            print(
                f"[Interpol] get_glyph_model: Using special layers - {len(source_layers)} sources "
                f"(masters: {num_masters}, uses_special: {uses_special_layers})"
            )

        # Validate locations are all unique
        location_tuples = [tuple(sorted(loc.items())) for loc in source_locations]
        if len(location_tuples) != len(set(location_tuples)):
            debug_log(
                "get_glyph_model: Duplicate locations found, falling back to master model"
            )
            # Fall back to master-only model
            if self.model is None:
                return None, None, None, None, None
        ...
        # Build glyph-specific model
        try:
            if DEBUG_INTERPOLATE:
                print(
                    f"[Interpol] get_glyph_model: Building VariationModel with locations: {source_locations}"
                )
            glyph_model = VariationModel(source_locations)
            if DEBUG_INTERPOLATE:
                print(
                    f"[Interpol] get_glyph_model: VariationModel created successfully"
                )
        except Exception as e:
            if DEBUG_INTERPOLATE:
                print(f"[Interpol] get_glyph_model: Failed to build glyph model: {e}")
            debug_log(f"get_glyph_model: Failed to build glyph model: {e}")
            return None, None, None, None, None

        # Extract points from all source layers
        glyph_points = {}
        widths = []
        decomposed_layers = []

        for ix, layer in enumerate(source_layers):
            decomposed = GlyphInterpolator.get_decomposed_layer(layer)
            decomposed_layers.append(decomposed)
            glyph_points[ix] = GlyphInterpolator.extract_points_from_layer(decomposed)
            widths.append(layer.width)
        ...
        if not GlyphInterpolator.validate_points_compatibility(glyph_points):
            if DEBUG_INTERPOLATE:
                print(
                    f"[Interpol] get_glyph_model: Incompatible sources - points don't match"
                )
                for idx, pts in glyph_points.items():
                    print(f"[Interpol]   source {idx}: {len(pts)} points")
            debug_log(
                "get_glyph_model: Incompatible sources, falling back to master model"
            )
            # Fall back to master-only if special layers are incompatible
            if self.model is None:
                return None, None, None, None, None
        ...
        # Get scalars for the glyph-specific model
        scalars = glyph_model.getMasterScalars(self.current_location)

        if DEBUG_INTERPOLATE:
            print(
                f"[Interpol] get_glyph_model: Built model with {len(source_layers)} sources, "
                f"{len(scalars)} scalars at location {self.current_location}"
            )
            print(f"[Interpol]   scalars: {scalars}")

        debug_log(
            f"get_glyph_model: Built model with {len(source_layers)} sources, "
            f"{len(scalars)} scalars at location {self.current_location}"
        )

        return glyph_model, glyph_points, widths, scalars, decomposed_layers[0]
    ...
    def update_glyph(self):
        ...
        This method now supports intermediate and alternate layers:
        - Intermediate layers: Additional design sources that participate in interpolation
        - Alternate layers: Shape switching at axis threshold boundaries
        """
        # Get font reference
        font = self._font if self._font else Glyphs.font
        if not font:
        ...
        if DEBUG_INTERPOLATE and self.current_location and self.axis_min_max:
            loc_info = [
                f"{tag}={self.get_actual_value(tag):.1f}"
                for tag in self.current_location
                if tag in self.axis_min_max
            ]
        ...
        try:
            ...
            # Get the appropriate model and data for this glyph
            # This handles intermediate and alternate layers automatically
            glyph_model, glyph_points, widths, scalars, current_decomposed = (
                self.get_glyph_model(glyph, font)
            )

            if glyph_model is None or glyph_points is None:
                # Fall back to basic master-only approach for compatibility
                debug_log(
                    "update_glyph(): get_glyph_model returned None, using fallback"
                )

                if not self.master_scalars:
                    debug_log("update_glyph(): No master_scalars, skipping")
                    return

                # Get decomposed layer for first master (consistent reference structure)
                first_master_layer = glyph.layers[font.masters[0].id]
                current_decomposed = GlyphInterpolator.get_decomposed_layer(
                    first_master_layer
                )
                ...
                if not GlyphInterpolator.validate_points_compatibility(
                    self.glyph_points
                ):
                    return

                glyph_points = self.glyph_points
                glyph_model = self.model
                scalars = self.master_scalars
                widths = [
                    layer_widths.get(i, current_layer.width)
                    for i in range(len(scalars))
                ]

            # Validate that points and scalars match
            all_points = list(glyph_points.values())
            if len(scalars) != len(all_points):
                debug_log(
                    f"update_glyph(): scalars ({len(scalars)}) != points ({len(all_points)})"
                )
                return

            # Interpolate points
            interpolated_points = GlyphInterpolator.interpolate_points(
                glyph_points, glyph_model, scalars
            )
            if interpolated_points is None:
                return

            # Interpolate the width
            self.interpolated_width = GlyphInterpolator.interpolate_width(
                widths, scalars
            )

            # Build bezier path with node tracking
            self.displaypath, self.interp_nodes, self.interp_handles = (
                GlyphInterpolator.build_bezier_path(
                    current_decomposed, interpolated_points
                )
            )
            ...
            if self.show_kinks and self.potential_kinks:
                ...
                node_to_point = GlyphInterpolator.build_node_to_point_map(
                    current_decomposed
                )

                for kink_data in self.potential_kinks:
                    orig_x, orig_y, max_angle_diff, node_id, has_ignored_axes = (
                        kink_data
                    )
                    ...
                    if (
                        prev_flat_idx is not None
                        and next_flat_idx is not None
                        and prev_flat_idx < len(interpolated_points)
                        and next_flat_idx < len(interpolated_points)
                    ):
                        ...
                        current_severity = KinkDetector.calculate_kink_severity(
                            p1, p2, p3
                        )
                    ...
                    current_angle_deg = KinkDetector.severity_to_degrees(
                        current_severity
                    )
            ...
            # Interpolate anchors
            # NOTE: Currently anchors use master-only scalars for simplicity.
            # TODO: Enhance to support glyph-specific sources (intermediate/alternate layers)
            glyph = current_layer.parent
    ...
    def _detect_potential_kinks(self):
        ...
        try:
            ...
            self.potential_kinks = KinkDetector.find_potential_kinks(
                current_layer, font
            )
    ...
    def get_interpolated_path_for_glyph(self, glyph):
        ...
        Supports alternate and intermediate layers when the respective settings are enabled.
        """
        if not self.master_scalars:
            return None, 0
        ...
        try:
            ...
            # Include layer settings in content hash to bust cache when toggled
            layers_flag = (self.use_intermediate_layers, self.use_alternate_layers)
            content_hash = self._cache.get_glyph_content_hash(glyph, font) + layers_flag
            ...
            # Check if we should try special layers (need axis_min_max and current_location)
            use_special = (
                (self.use_intermediate_layers or self.use_alternate_layers)
                and self.axis_min_max
                and self.current_location
            )

            if use_special:
                # Try to get glyph-specific model with special layers
                try:
                    model, glyph_points, widths, scalars, reference_decomposed = (
                        self.get_glyph_model(glyph, font)
                    )

                    if (
                        model is not None
                        and glyph_points is not None
                        and scalars is not None
                    ):
                        all_points = list(glyph_points.values())

                        # Handle glyphs with no paths (like space)
                        if len(all_points) > 0 and len(all_points[0]) == 0:
                            interp_width = GlyphInterpolator.interpolate_width(
                                widths, scalars
                            )
                            result = (None, interp_width)
                            self._cache.set_path(
                                glyph_id, scalars_hash, content_hash, result
                            )
                            return result

                        # Interpolate points using the glyph-specific model and scalars
                        interpolated_points = GlyphInterpolator.interpolate_points(
                            glyph_points, model, scalars
                        )
                        if interpolated_points is not None:
                            ...
                            # Build bezier path
                            displaypath = GlyphInterpolator.build_bezier_path_simple(
                                reference_decomposed, interpolated_points
                            )
                    ...
                    if DEBUG_INTERPOLATE:
                        ...
                        print(
                            f"[Interpol] get_interpolated_path_for_glyph special layers exception: {e}"
                        )
                        traceback.print_exc()
                    pass  # Fall through to standard interpolation

            # Standard master-only interpolation
            glyph_points, widths, reference_layer = (
                GlyphInterpolator.extract_master_data(glyph, font)
            )
            ...
            displaypath = GlyphInterpolator.build_bezier_path_simple(
                reference_layer, interpolated_points
            )
            ...
            if DEBUG_INTERPOLATE:
                ...
                print(
                    f"[Interpol] get_interpolated_path_for_glyph outer exception: {e}"
                )
                traceback.print_exc()
            return None, 0
        ...
        try:
            ...
            try:
                ...
                for i, layer in enumerate(allLayers):
                    if (
                        layer
                        and not is_newline_layer(layer)
                        and layer.parent == selectedGlyph
                    ):
                        selectedIndex = i
                        break
...
class AxisSlider(Group):
    def __init__(
        self, axis, min_value, max_value, posSize=None, owner=None, *args, **kwargs
    ):
    ...
    def set_speed_label(self, speed: float):
        ...
        try:
            ...
            self.speedBox.getNSComboBox().setStringValue_(
                self.speedBox.getNSComboBox().itemObjectValueAtIndex_(idx)
            )
        except Exception:
            pass
...
class InterpolSettings:
    ...
    def _build_window(self):
        ...
        self.w.shortcutLabel = TextBox(
            (margin, y + 3, label_width, 20), "Tool Shortcut Key:"
        )
        ...
        self.w.previewColorsTitle = TextBox(
            (margin, y, -margin, 20), "Interpolation Preview", sizeStyle="small"
        )
        y += 25

        # Fill color
        self.w.fillColorLabel = TextBox(
            (margin, y + 12, label_width, 20), "Fill Color:"
        )
        ...
        self.w.outlineColorLabel = TextBox(
            (margin, y + 12, label_width, 20), "Outline / Nodes / Handles:"
        )
        ...
        self.w.kinkIndicatorColorLabel = TextBox(
            (margin, y + 12, label_width, 20), "Kink Indicator:"
        )
        kink_indicator_color = InterpolateConfig.get_kink_indicator_color()
        ...
        # --- Line Style ---
        self.w.lineStyleLabel = TextBox((margin, y + 2, label_width, 20), "Line Style:")
        ...
        self.w.dekinkTitle = TextBox(
            (margin, y, -margin, 20), "Dekink Window Preview", sizeStyle="small"
        )
        y += 25
    ...
    def _on_reset_defaults(self, sender):
        ...
        self.w.fillColorWell.set(
            NSColor.colorWithRed_green_blue_alpha_(*DEFAULT_FILL_COLOR)
        )
        _save_color_to_defaults(KEY + ".color.fill", DEFAULT_FILL_COLOR)

        self.w.outlineColorWell.set(
            NSColor.colorWithRed_green_blue_alpha_(*DEFAULT_OUTLINE_COLOR)
        )
        _save_color_to_defaults(KEY + ".color.outline", DEFAULT_OUTLINE_COLOR)

        self.w.kinkIndicatorColorWell.set(
            NSColor.colorWithRed_green_blue_alpha_(*DEFAULT_KINK_INDICATOR_COLOR)
        )
        _save_color_to_defaults(
            KEY + ".color.kink_indicator", DEFAULT_KINK_INDICATOR_COLOR
        )
        ...
        self.w.dekinkPreviewColorWell.set(
            NSColor.colorWithRed_green_blue_alpha_(*DEFAULT_DEKINK_PREVIEW_COLOR)
        )
        _save_color_to_defaults(
            KEY + ".color.dekink_preview", DEFAULT_DEKINK_PREVIEW_COLOR
        )
...
class InterpolateReporter(ReporterPlugin):
    ...
    def _get_state_for_current_font(self):
        ...
        if state is None:
            debug_log(
                f"Reporter._get_state_for_current_font(): No state found for '{font_sig}'"
            )
        ...
        try:
            ...
            if Glyphs.buildNumber >= 3320:
                ...
                newMenuItem = MenuItem(
                    "Interpol Settings…", action=self.showSettings_, target=self
                )
            else:
    ...
    def conditionalContextMenus(self):
        ...
        def menu_item(name, action, checked=False):
            return {"name": name, "action": action, "state": checked}

        label = NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(
            "Interpol Preview Settings", None, ""
        )
        label.setEnabled_(False)  # non-clickable
        label.setAttributedTitle_(
            NSAttributedString.alloc().initWithString_attributes_(
                "Interpol Preview Settings",
        ...
        contextMenus.append(
            menu_item("Show Fill", self.toggleShowFill_, state.show_fill)
        )
        contextMenus.append(
            menu_item("Show Outlines/Points", self.toggleShowNodes_, state.show_nodes)
        )
        contextMenus.append(
            menu_item("Show Anchors", self.toggleShowAnchors_, state.show_anchors)
        )
        contextMenus.append(
            menu_item("Show Kinks", self.toggleShowKinks_, state.show_kinks)
        )
        contextMenus.append({"menu": NSMenuItem.separatorItem()})
        contextMenus.append(
            menu_item("Dim Layer", self.toggleDimLayer_, state.dim_layer)
        )
        contextMenus.append(
            menu_item("Follow masters", self.toggleFollowMasters_, state.follow_masters)
        )
        contextMenus.append(
            menu_item(
                "Spacebar shows interpolation",
                self.toggleSpacePreviewInterpolated_,
                state.space_preview_interpolated,
            )
        )
        contextMenus.append({"menu": NSMenuItem.separatorItem()})
        contextMenus.append(
            menu_item(
                ...
                self.toggleUseIntermediateLayers_,
                state.use_intermediate_layers,
            )
        )
        contextMenus.append(
            menu_item(
                "Use Alternate Layers",
                self.toggleUseAlternateLayers_,
                state.use_alternate_layers,
            )
        )
        contextMenus.append({"menu": NSMenuItem.separatorItem()})
        contextMenus.append(
            menu_item(
                "Preview Window", self.togglePreviewWindow_, state.show_preview_window
            )
        )
        contextMenus.append(
            menu_item(
                "Preview Bubble", self.togglePreviewBubble_, state.show_compare_line
            )
        )
        ...
        try:
            ...
            slider.setAction_(
                objc.selector(self.bubbleSizeSliderChanged_, signature=b"v@:@")
            )
        ...
        try:
            ...
            if font and font.selectedLayers:
                layer = font.selectedLayers[0]
                selected_smooth_nodes = (
                    SynchronizationHelper.get_selected_smooth_oncurve_nodes(layer)
                )
                if selected_smooth_nodes:
                    contextMenus.append(
                        menu_item("Dekink window...", self.openSyncRatiosPanel_, False)
                    )
                    contextMenus.append({"menu": NSMenuItem.separatorItem()})
        except Exception:
            pass
    ...
    def toggleUseIntermediateLayers_(self, sender=None):
        state = self._get_state_for_current_font()
        if not state:
            return
        state.use_intermediate_layers = not state.use_intermediate_layers
        state.save_defaults()
        # Clear caches to force re-computation
        state.clear_caches(clear_kinks=True)
        Glyphs.redraw()

    def toggleUseAlternateLayers_(self, sender=None):
        state = self._get_state_for_current_font()
        if not state:
            return
        state.use_alternate_layers = not state.use_alternate_layers
        state.save_defaults()
        # Clear caches to force re-computation
        state.clear_caches(clear_kinks=True)
        Glyphs.redraw()
        ...
        try:
            ...
            if superview:
                for sub in superview.subviews():
                    try:
                        if (
                            hasattr(sub, "identifier")
                            and sub.identifier() == "bubbleSizeValue"
                        ):
        ...
        for path in layer.paths:
            for node in path.nodes:
                ...
                if node.type == "offcurve":
                    ...
                    nodeRect = NSMakeRect(
                        x - nodeRadius, y - nodeRadius, nodeRadius * 2, nodeRadius * 2
                    )
    ...
    def _draw_interpolation_preview(self, layer, state):
        ...
        if DEBUG_INTERPOLATE and state.current_location and state.axis_min_max:
            loc_info = [
                f"{tag}={state.get_actual_value(tag):.1f}"
                for tag in state.current_location
                if tag in state.axis_min_max
            ]
            print(
                f"[Interpol] Reporter.foreground(): Drawing at location: {', '.join(loc_info)}, state id={id(state)}"
            )
        ...
        if show_nodes and getattr(state, "interp_nodes", None):
            ...
            for on_curve, pt in state.interp_nodes:
                ...
                # Draw center dot
                DrawingHelpers.draw_circle(
                    (centeredX, pt.y), 1.0, fill_color=nodeColor, scale=scale
                )
        ...
        if show_anchors and getattr(state, "interpolated_anchors", None):
            ...
            for name, pt in state.interpolated_anchors.items():
                ...
                DrawingHelpers.draw_cross(
                    (centeredX, pt[1]), 5.0, anchorColor, stroke_width=1.25, scale=scale
                )
    ...
    def _draw_sync_preview(self, layer, state, scale, offsetX):
        ...
        try:
            ...
            first_master_layer = glyph.layers[font.masters[0].id]
            reference_decomposed = GlyphInterpolator.get_decomposed_layer(
                first_master_layer
            )

            # Build the preview path with node/handle tracking (same as main interpolation preview)
            preview_path, sync_nodes, sync_handles = (
                GlyphInterpolator.build_bezier_path(
            ...
            self._draw_sync_preview_handles_and_nodes(
                sync_nodes, sync_handles, offsetX, scale, dekink_colors
            )
    ...
    @objc.python_method
    def _draw_sync_preview_handles_and_nodes(
        self, sync_nodes, sync_handles, offsetX, scale, colors=None
    ):
        ...
        try:
            ...
            for on_curve, pt in sync_nodes:
                ...
                # Draw center dot
                DrawingHelpers.draw_circle(
                    (pt.x + offsetX, pt.y), 1.0, fill_color=nodeColor, scale=scale
                )
        ...
        try:
            ...
            for i in range(cursorIndex - 1, lineStart - 1, -1):
                ...
                if i + 1 <= cursorIndex:
                    ...
                    if (
                        nextLyr
                        and not is_newline_layer(nextLyr)
                        and lyr.parent
                        and nextLyr.parent
                    ):
                        # Get kerning from current master
                        kern = font.kerningForPair(
            ...
            for i, lyr in enumerate(lineLayers):
                ...
                if i > 0:
                    ...
                    if (
                        prevLyr
                        and not is_newline_layer(prevLyr)
                        and prevLyr.parent
                        and glyph
                    ):
                        kern = font.kerningForPair(
            ...
            selectedLayer = (
                allLayers[cursorIndex] if cursorIndex < len(allLayers) else None
            )
            ...
            if selectedGlyphX is not None and selectedLayerWidth:
                ...
                interpCenter = (
                    selectedGlyphX
                    + (selectedGlyphInterpWidth or (selectedLayerWidth * scale)) * 0.5
                )
    ...
    def _get_current_line_info(self, tab, cursorIndex):
        ...
        lineLayers = [
            l for l in allLayers[lineStart:lineEnd] if not is_newline_layer(l)
        ]
        if not lineLayers:
            return None
    ...
    def foregroundInViewCoords(self):
        ...
        try:
                ...
                if lyr and not is_newline_layer(lyr):
                    ...
                    if i + 1 < len(allLayers) and i + 1 <= cursorIndex:
                        ...
                        if (
                            ...
                            kern = state.get_interpolated_kerning(
                                lyr.parent, nextLyr.parent
                            )
                            lineStartX -= kern * scale
            ...
            horizontalMargin = (
                100 * scale
            )  # Generous horizontal margins for breathing room
...
class InterpolatePalette(PalettePlugin):
    ...
    def populate_axis_data(self, font=None):
        ...
        if not font or not font.axes:
            debug_log(
                f"populate_axis_data(): No font or no axes (font={font is not None}, axes={len(font.axes) if font and font.axes else 0})"
            )
            return
        ...
        masters = font.masters
        debug_log(
            f"populate_axis_data(): Populating for font '{font_name}' with {len(axes)} axes"
        )

        self.state.axis_min_max = {}
        axis_tags = []  # Collect axis tags for default tool assignment
        for i, axis in enumerate(axes):
            min_value = min(master.internalAxesValues[i] for master in masters)
            max_value = max(master.internalAxesValues[i] for master in masters)
        ...
        if self.state.tool_horizontal_axis is None and len(axis_tags) >= 1:
            ...
            debug_log(
                f"populate_axis_data(): Default horizontal axis set to '{axis_tags[0]}'"
            )
        if self.state.tool_vertical_axis is None and len(axis_tags) >= 2:
            self.state.tool_vertical_axis = axis_tags[1]
            debug_log(
                f"populate_axis_data(): Default vertical axis set to '{axis_tags[1]}'"
            )
    ...
    def _build_ui(self, axes_data):
        ...
        if axes_data:
            ...
            group.toolMappingLabel = TextBox(
                (left_margin, y, -0, row_height), "Tool Axis Mapping:", sizeStyle="mini"
            )
            y += row_height

            axis_options = ["None"] + [
                f"{axis.name} ({axis.axisTag})" for axis, _, _ in axes_data
            ]
            ...
            group.horizontalAxisLabel = TextBox(
                (left_margin, y, 35, row_height), "X axis:", sizeStyle="mini"
            )

            h_axis_index = 0
            if (
                self.state.tool_horizontal_axis
                and self.state.tool_horizontal_axis in self._tool_axis_tags
            ):
                h_axis_index = self._tool_axis_tags.index(
                    self.state.tool_horizontal_axis
                )
            ...
            group.verticalAxisLabel = TextBox(
                (left_margin, y, 35, row_height), "Y axis:", sizeStyle="mini"
            )

            v_axis_index = 0
            if (
                self.state.tool_vertical_axis
                and self.state.tool_vertical_axis in self._tool_axis_tags
            ):
    ...
    def rebuild_axis_sliders(self):
        ...
        if rebuild_needed:
            debug_log(
                f"rebuild_axis_sliders(): Rebuilding for {num_axes} axes (was {current_count})"
            )

            # Build axis data list
            axes_data = []
            for axis in current_axes:
                min_value, max_value = self.state.axis_min_max.get(
                    axis.axisTag, (0, 1000)
                )
    ...
    def update_position(self, axis, value):
        ...
        if axis_tag not in self.state.axis_min_max:
            ...
            if axis_tag not in self.state.axis_min_max:
                ...
                debug_log(
                    f"  -> Axis {axis_tag} STILL not in axis_min_max after repopulate!"
                )
                return
        ...
        debug_log(
            f"  -> Normalized: current_location[{axis_tag}] = {self.state.current_location[axis_tag]}"
        )
        if self.state.model:
            self.state.master_scalars = self.state.model.getMasterScalars(
                self.state.current_location
            )
            debug_log(
                f"  -> Updated master_scalars, count={len(self.state.master_scalars)}"
            )
        else:
    ...
    def _refresh_global_play_button(self):
        try:
            ...
            if hasattr(group, "globalPlayButton"):
                any_playing = (
                    any(
                        a.get("is_playing", False)
                        for a in self.state.axis_animation.values()
                    )
                    if self.state.axis_animation
                    else False
                )
    ...
    def togglePlayAll(self, sender):
        ...
        now = time.time()
        any_playing = (
            any(a.get("is_playing", False) for a in self.state.axis_animation.values())
            if self.state.axis_animation
            else False
        )
    ...
    def horizontalAxisChanged(self, sender):
        ...
        try:
            ...
            if index < len(tool_axis_tags):
                ...
                debug_log(
                    f"Horizontal axis changed to: {self.state.tool_horizontal_axis}"
                )
        except Exception:
            pass
    ...
    def update(self, sender):
        ...
        glyph_name = (
            selected_layers[0].parent.name if selected_layers[0].parent else "Unknown"
        )
        ...
        debug_log(
            f"update() palette={id(self)}, font='{font_name}', state={id(self.state)}, sig={new_font_sig}"
        )
        debug_log(
            f"  -> state.axis_min_max keys: {list(self.state.axis_min_max.keys()) if self.state.axis_min_max else []}"
        )
        debug_log(
            f"  -> state.model: {self.state.model is not None}, master_scalars: {len(self.state.master_scalars) if self.state.master_scalars else 0}"
        )
        debug_log(
            f"  -> _initialized_for_font: {getattr(self, '_initialized_for_font', False)}, _instance_font_sig: {getattr(self, '_instance_font_sig', None)}"
        )
        ...
        if not getattr(self, "_initialized_for_font", False):
            debug_log(
                f"  -> FIRST INITIALIZATION for this palette with font '{font_name}'"
            )
            ...
            self.populate_axis_data()
            debug_log(
                f"  -> axis_min_max after initial populate: {self.state.axis_min_max}"
            )
            ...
            debug_log(
                f"  -> model built, master_scalars={len(self.state.master_scalars) if self.state.master_scalars else 0}"
            )
            ...
            if font_changed:
                ...
                self.populate_axis_data()
                debug_log(
                    f"  -> axis_min_max after populate: {self.state.axis_min_max}"
                )
                ...
                self.state.build_model()
                debug_log(
                    f"  -> model rebuilt, master_scalars={len(self.state.master_scalars) if self.state.master_scalars else 0}"
                )
        ...
        try:
            ...
            if current_master_id != getattr(self.state, "_last_master_id", None):
                ...
                if getattr(self.state, "follow_masters", False) and current_master:
                    applied = self.state.set_location_from_master(
                        current_master, font.axes
                    )
                    if applied:
                        ...
                        try:
                            ...
                            for axis in font.axes:
                                if axis.axisTag in self.state.current_location:
                                    updates[axis.axisTag] = self.state.get_actual_value(
                                        axis.axisTag
                                    )
                            self._sync_axis_controls(updates)
                        except Exception:
        ...
        if (
            ...
            self.state.clear_caches(
                clear_kinks=True
            )  # Clear all caches including kinks for new glyph
        ...
        debug_log(
            f"update(): Calling update_glyph() - model={self.state.model is not None}, master_scalars={len(self.state.master_scalars) if self.state.master_scalars else 0}, axis_min_max keys={list(self.state.axis_min_max.keys())}"
        )
        self.state.update_glyph()
...
class InterpolateTool(SelectTool):
    ...
    @objc.python_method
    def settings(self):
        self.name = Glyphs.localize({"en": "Interpol Tool"})
        ...
        icon_path = os.path.join(
            os.path.dirname(self.__file__()), "toolbarIconTemplate.pdf"
        )
        ...
        self.generalContextMenus = [
            {
                "name": Glyphs.localize({"en": "Reset to center"}),
    ...
    def mouseDown_(self, theEvent):
        ...
        try:
            ...
            debug_log(
                f"InterpolateTool.mouseDown_: start={self._drag_start_location}, axes={self._drag_start_axis_values}"
            )
        except Exception as e:
    ...
    def mouseDragged_(self, theEvent):
        ...
        try:
            ...
            debug_log_lazy(
                lambda state=state: f"InterpolateTool.mouseDragged_: Using state id={id(state)}"
            )
            ...
            if h_axis and h_axis in state.axis_min_max:
                ...
                debug_log_lazy(
                    lambda h=h_axis, n=new_val, a=state.get_actual_value(
                        h_axis
                    ): f"InterpolateTool: h_axis={h}, norm={n:.3f}, actual={a:.1f}"
                )
                changed = True
            ...
            if v_axis and v_axis in state.axis_min_max:
                ...
                debug_log_lazy(
                    lambda v=v_axis, n=new_val, a=state.get_actual_value(
                        v_axis
                    ): f"InterpolateTool: v_axis={v}, norm={n:.3f}, actual={a:.1f}"
                )
                changed = True
            ...
            if changed:
                ...
                if state.model:
                    state.master_scalars = state.model.getMasterScalars(
    ...
    def _sync_palette_sliders(self):
        ...
        try:
            ...
            if not palette:
                debug_log(
                    "InterpolateTool._sync_palette_sliders: No palette reference in state"
                )
                return
    ...
    def _update_palette_sliders(self, palette, state):
        ...
        try:
            ...
            for slot in palette._axis_slots:
                ...
                if (
                    axis_tag in state.current_location
                    and axis_tag in state.axis_min_max
                ):
    ...
    def doubleClickAtPoint_OnNode_OnPath_OnAnchor_OnGuideLine_OnComponent_OnText_(
        ...
        try:
            objc.super(
                InterpolateTool, self
            ).doubleClickAtPoint_OnNode_OnPath_OnAnchor_OnGuideLine_OnComponent_OnText_(
    ...
    def foreground(self, layer):
        ...
        try:
            ...
            if (
                h_axis
                and h_axis in state.axis_min_max
                and h_axis in state.current_location
            ):
            ...
            if (
                v_axis
                and v_axis in state.axis_min_max
                and v_axis in state.current_location
            ):
